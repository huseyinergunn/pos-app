...
import { PrinterOutlined, CloseCircleOutlined, ReloadOutlined, FileTextOutlined, CreditCardOutlined, WalletOutlined } from "@ant-design/icons";
...
const BillPage = () => {
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white font-black shadow-lg uppercase">
            {text?.charAt(0)}
          </div>
    ...
    },
    {
      ...
      render: (status) => {
        ...
        return (
          <div className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${isCancelled ? "bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500" : "bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400"}`}>
            {status}
          </div>
        );
      },
        ...
        <Space size="small">
          <Tooltip title="Görüntüle / Yazdır">
            <Button 
              type="text" 
              size="large" 
              className="hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 rounded-xl" 
              icon={<PrinterOutlined />} 
              onClick={() => { setSelectedBill(record); setIsPrintModalOpen(true); }}
            />
          </Tooltip>
    ...
    return (
      <div className="h-[80vh] flex items-center justify-center bg-transparent">
        <Result
          ...
          extra={<Button type="primary" size="large" className="h-12 px-10 rounded-2xl font-bold uppercase tracking-wider shadow-lg shadow-blue-500/20 border-none bg-blue-600 hover:bg-blue-700 transition-all" onClick={() => navigate("/")}>Ana Sayfaya Dön</Button>}
        />
      </div>
    );
  }
  ...
  <Table 
    ...
    pagination={{ 
      pageSize: 8, 
      className: "px-8 py-6 m-0 flex justify-center md:justify-end",
    }} 
    scroll={{ x: 800 }} 
  />
</div>
        {isPrintModalOpen && (
          <PrintBill 
            ...
            setIsModalOpen={(val) => { 
              setIsPrintModalOpen(val); 
              if(!val) setSelectedBill(null); 
            }} 
