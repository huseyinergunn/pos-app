...
const ProductItem = ({ item }) => {
  ...
  const handleClick = (e) => {
    ...
    message.success({
      content: cartItem ? `${item.title} artırıldı` : `${item.title} eklendi`,
      duration: 1.5,
      key: "cart-update",
      style: { marginTop: '8vh' },
    });
  ...
  return (
    <div
      className={`group relative bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-white/40 dark:border-slate-800/50 rounded-[2.5rem] transition-all duration-300 cursor-pointer flex flex-col h-full z-10 lg:hover:z-20 lg:hover:-translate-y-2 select-none touch-manipulation 
    ...
    >
      <div className="relative aspect-square m-2.5 overflow-hidden rounded-[1.8rem] bg-gradient-to-br from-slate-50/50 to-slate-200/30 dark:from-slate-800/40 dark:to-slate-900/60 flex items-center justify-center p-4">
        <img
          src={item.img}
          alt={item.title}
          className={`object-contain w-full h-full transition-transform duration-500 pointer-events-none drop-shadow-2xl 
            ${isPressed || "lg:group-hover:scale-110"}`}
        />
        
        <div className={`absolute inset-0 bg-blue-600/30 flex items-center justify-center backdrop-blur-[4px] pointer-events-none transition-all duration-300 
          ${isPressed ? "opacity-100" : "opacity-0 lg:group-hover:opacity-100"}`}>
            <div className={`bg-white text-blue-600 w-12 h-12 rounded-full flex items-center justify-center shadow-2xl transition-transform duration-300 
              ${isPressed ? "translate-y-0 scale-110" : "translate-y-4 lg:group-hover:translate-y-0"}`}>
      ...
      <div className="px-5 pb-5 pt-1 flex flex-col flex-1 relative pointer-events-none">
        <div className="mb-3">
          <span className="text-[9px] font-black text-blue-500/60 uppercase tracking-[0.2em]">Ürün Detayı</span>
          <h3 className={`text-[13px] font-black leading-tight transition-colors uppercase tracking-tight line-clamp-2 mt-1 
            ${isPressed ? "text-blue-600" : "text-slate-800 dark:text-slate-100 lg:group-hover:text-blue-600"}`}>
            {item.title}
          </h3>
        </div>
